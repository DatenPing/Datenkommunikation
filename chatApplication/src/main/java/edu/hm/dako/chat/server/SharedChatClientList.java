...
public class SharedChatClientList {
    ...
    public synchronized void deleteClientWithoutCondition(String userName) {
        ...
        log.debug(String.format("Client %s zwangsweise aus allen Listen entfernen", userName));
        for (String s : new HashSet<String>(clients.keySet())) {
            ClientListEntry client = (ClientListEntry) clients.get(s);
            if (client.getWaitList().contains(userName)) {
        ...
        log.debug(String.format("Client %s vollstaendig aus allen Wartelisten entfernt", userName));
    }
    ...
    public synchronized boolean deleteClient(String userName) {
        ...
        log.debug(
                String.format("Logout fuer %s, Laenge der Clientliste vor dem Loeschen von: %s: %d", userName, userName, clients.size()));
        ...
        if (removeCandidateClient != null) {
            ...
            if ((removeCandidateClient.getWaitList().size() == 0)
                ...
                // Wartelisten ist
                log.debug(String.format("Warteliste von Client %s ist leer und Client ist zum Beenden vorgemerkt", removeCandidateClient.getUserName()));

                for (String s : new HashSet<String>(clients.keySet())) {
                    ClientListEntry client = (ClientListEntry) clients.get(s);
                    if (client.getWaitList().contains(userName)) {
                        log.debug(String.format("Loeschen nicht moeglich, da Client %s noch in der Warteliste von %s ist", userName, s));
                        return deletedFlag;
                    }
                }
        ...
        log.debug(String.format("Laenge der Clientliste nach dem Loeschen von %s: %d", userName, clients.size()));
        log.debug(String.format("Clientliste nach dem Loeschen von %s: %s", userName, printClientList()));
        return deletedFlag;
    }
    ...
    public synchronized Vector<String> gcClientList() {
        ...
        for (String s1 : new Vector<String>(clients.keySet())) {
            ...
            if (!clientUsed) {
                log.debug(String.format("Garbace Collection: Client %s wird aus ClientListe entfernt", client1.getUserName()));
    ...
    public synchronized void incrNumberOfReceivedChatEventConfirms(String userName) {
        // TODO Ben: Mit der methode arbeiten, um die Messages zu bestätigen
        ClientListEntry client = clients.get(userName);
        if (client != null) {
     ...
     *
     * @param userName  Name des Clients, fuer den ein Listeneintrag aus seiner Warteliste
     *                  geloescht werden soll
     * @param entryName name des Clients, der aus der Event-Warteliste geloescht werden
     *                  soll
     * @return Anzahl der noch vorhandenen Eintraege in der Liste
     * @throws Exception Client mit UserName existiert nicht
     */
